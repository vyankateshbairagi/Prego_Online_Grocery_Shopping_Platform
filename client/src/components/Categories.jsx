...
const Categories = () => {
  ...
  const { navigate, t, getCategoryLabel } = useAppContext()

  return (
    <div className='mt-16'>
      <p className='text-2xl md:text-3xl font-medium'>{t("categories.title")}</p>
      <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-6 mt-6 gap-6'>
        ...
        {categories.map((category, index) => (
          ...
          >
            ...
            <p className='text-sm font-medium'>{getCategoryLabel(category.path)}</p>
          </div>
